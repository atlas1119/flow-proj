...
import {User,Flow, Node, WorkNode ,TemplateNode, Chain, ProfitReport, ProfitDetail, CostInfo} from '../model';
...
router.get('/business/editnode', async(ctx, next)=>{
    ...
    if(ctx.session.user){
        ...
        // 根据user id来获取代码
        var manage_first_user = await User.findById(selected_work_node.person_manage_id);
        await ctx.view('business_manage',{edit,node,work_nodes,manage_first_user,selected_work_node});
