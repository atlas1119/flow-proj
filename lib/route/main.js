...
import {User,Flow, Node, WorkNode ,TemplateNode, Chain, ProfitReport, ProfitDetail} from '../model';
...
router.get('/report', async(ctx, next)=>{
    ...
    if(sess && sess.user){
        var profits = await ProfitDetail.find({}).exec();
        ...
        await ctx.view('report',{profitdata});
    }else{
        ctx.redirect('/login');
    }
...
router.get('/report/detail', async(ctx, next)=>{
    ...
    if(sess && sess.user){
        var profits = await ProfitDetail.find({}).exec();
        await ctx.view('report_detail',{profits});
    }else{
        ctx.redirect('/login');
    }
